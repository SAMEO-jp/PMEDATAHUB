...
export default function SetsubiAssignPage({ params }: AssignPageProps) {
  const router = useRouter();
  const setsubiId = parseInt(params.setsubi_id);



  const { data: setsubiDetail, isLoading: loadingSetsubi } = trpc.setsubi.getDetail.useQuery(
    { setsubi_id: setsubiId },
    { enabled: !isNaN(setsubiId) }
  );

  // プロジェクトメンバーを取得 (temporarily disabled due to router issues)
  const projectMembers: any[] = [];
  const loadingMembers = false;

  const assignMutation = trpc.setsubi.assignUser.useMutation();
  const removeMutation = trpc.setsubi.removeAssignment.useMutation();

  // プロジェクトメンバーのデータを正しく取得
  ...
  const isUserAssigned = (userId: string) => {
    return setsubiDetail?.data?.current_assignees?.some(assignee => assignee.user_id === userId) || false;
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-4">
        <ol className="flex items-center space-x-2 text-sm text-gray-500">
          ...
          <li>
            <button
              onClick={() => router.push(`/app_project/${params.project_id}/manage/setsubi/${setsubiId}`)}
              className="hover:text-blue-600 transition-colors"
            >
              {setsubi.seiban}
            </button>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {setsubi.setsubi_name} ({setsubi.seiban}) の担当者を管理します
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
              ...
              現在の担当者 ({setsubi.current_assignees?.length || 0})
            </CardTitle>
          </CardHeader>
          <CardContent>
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const assignment = setsubi.assignments?.find(a => a.user_id === assignee.user_id);
                            if (assignment) {
          ...
          <CardContent>
            {loadingMembers ? (
              ...
              <Table>
                ...
                <TableBody>
                  {members.map((member) => (
                    <TableRow key={member.user?.user_id || member.user_id}>
                      <TableCell className="font-medium">
                        {member.user?.name_japanese || member.name_japanese}
                      </TableCell>
                      <TableCell>
                        {member.user?.bumon || member.bumon} {member.user?.sitsu || member.sitsu} {member.user?.ka || member.ka}
                      </TableCell>
                      <TableCell>
                        {isUserAssigned(member.user?.user_id || member.user_id) ? (
                      ...
                      <TableCell>
                        {isUserAssigned(member.user?.user_id || member.user_id) ? (
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleAssignUser(member.user?.user_id || member.user_id)}
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">プロジェクトメンバーが見つかりません</p>
              </div>
            )}
          </CardContent>
        </Card>
