...
export function formatEventForClient(item: BaseEvent): DisplayEvent {
  ...
  const height = (durationMs / (1000 * 60 * 60)) * 64;
  ...
  const displayProps = {
    top,
    height,
    originalHeight: height,  // 新規作成時の元の高さ（既存データの場合はheightと同じ値）
...
export function formatEventForServer(event: DisplayEvent): BaseEvent {
  ...
  const { height, top, color, unsaved, originalHeight, ...serverEvent } = event;
  return {
    ...serverEvent,
