...
interface UserSetsubiAssignPageProps {
  params: {
    user_id: string;
    project_id: string;
  };
...
export default function UserSetsubiAssignPage({ params }: UserSetsubiAssignPageProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const setsubiId = searchParams.get('setsubi_id');
  ...
  const handleCancel = () => {
    router.push(`/page/user/${params.user_id}/setsubi/add/${params.project_id}`);
  };
  ...
  }, [setsubiId, router, params.user_id, params.project_id]);

  // ローディング中
  if (userLoading || projectLoading || setsubiLoading) {
    return (
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-4">
        <ol className="flex items-center space-x-2 text-sm text-gray-500">
          ...
          <li className="text-gray-900 font-medium">{setsubi.setsubi_name}</li>
        </ol>
      </nav>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-md">
              ...
              <div>
                ...
                <p className="text-sm">{setsubi.setsubi_name} (製番: {setsubi.seiban})</p>
              </div>
            </div>
