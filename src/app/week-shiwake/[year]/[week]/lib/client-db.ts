 ...
 * @returns 実績データの配列
 */
export async function getAchievementsByWeek(year: number, week: number, user_id: string, forceRefresh = false) {
  try {
...
export async function saveWeekAchievements(year: number, week: number, events: any[]) {
  try {
    ...
    // APIを呼び出して週データを一括保存
    const response = await fetch(`/api/zisseki/week/${events[0].employeeNumber}/${year}/${week}`, {
      method: "POST",
      headers: {
...
export async function deleteAchievement(keyID: string) {
  try {
    ...
    const response = await fetch(`/api/zisseki/${keyID}`, {
      method: "DELETE",
    });
...
export async function getEmployees() {
  try {
    ...
    // 現在のユーザーを従業員リストとして返す
    return [{
      employeeNumber: parsedUser.user_id || parsedUser.employeeNumber,
      name: parsedUser.name || "未設定"
    }]
  } catch (error) {
